...
export const fileToArray = (url: string): Promise<ArrayBuffer> => new Promise<ArrayBuffer>((resolve, reject) => {
    try {
...
export const handleFileUpload = (fileInput: any) => new Promise<string>((resolve, reject) => {
...
const getBlobFromUrl = (url: string) => new Promise<File>((resolve, reject) => {
    const request = new XMLHttpRequest();
    request.open('GET', url, true);
    request.responseType = 'blob';
    request.onload = () => {
